...
def get_shimoneta(m):
	...
	if "リョナ" in m.content:
		...
		if random.randint(0, 99) < 50:
			quotes = [
				...
				[100, "かわいすぎると虐めたくなっちゃう現象を「キュートアグレッション」って言うんですって！"],
	...
	if "クリトリス" in m.content or "クリちゃん" in m.content or "クリ勃起" in m.content or "クリ責め" in m.content or "陰核" in m.content:
		quotes = [
			...
			[100, "クリトリスは敏感だから　乱暴にいじっちゃいけないんですよー！"]
		]
		return get_quotes(quotes)
	...
	if "お尻" in m.content or ("ケツ" in m.content and "バケツ" not in m.content):
		quotes = [
			[100, "お尻！"],
			[100, "お尻が大きくて安産型ならいくら孕ませても大丈夫ですね！"],
			[100, "ムチムチなお尻が好きなんですか？"],
	...
	if "百合" in m.content or "レズ" in m.content or "女の子同士" in m.content:
		quotes = [
			...
			[100, "メス交尾ってやつですよね"],
			[100, "美人のお姉さんとかだったらレイプされても・・・い、いいかも・・・"],
	...
	if "キンタマ" in m.content or "金玉" in m.content or "タマタマ" in m.content:
		quotes = [
			...
			[100, "タマタマを舐めて欲しいんですか？"],
			[100, "金玉って球体じゃなくてラグビーボールみたいな楕円形なんですって！"],
			[100, "金玉って50kg程度で潰れちゃうんですって！大切にしてあげてくださいね！"],
			[100, "急所なのにタマタマが外に出てるのは、精子が34度付近が一番生産しやすく活動的になるからなんですよ"],
			[100, "金玉って蹴られるとめっちゃ痛いんでしょー？どんな感じなんですか！"],
			[100, "左右で玉の大きさが違うから位置が逆にならないらしいですよ。でもソースは忘れちゃったので違っても知りません"]
		]
		return get_quotes(quotes)
	...
	if "腋" in m.content and "腋毛" not in m.content:
		quotes = [
			...
			[100, "腋部分が空いてる衣装ってエッチですよね・・・"],
			[100, "腋をペロペロしたいんですか？"],
			[100, "腋もエッチなので性器みたいなものなんですよ！"]
		]
		return get_quotes(quotes)
	...
	if "処女" in m.content:
		quotes = [
			[100, "処女膜って実は色んな形があるんですよ"],
			[100, "処女のままアナルを開発してあげたーい！"],
			[100, "私も処女ですよ！！"],
			[100, "せっかく処女を守ってきたのに、レイプとかで無理矢理散らされちゃうのいいですよね！"],
			[100, "ほそーい触手を使えば処女のまま開発できるのかなぁ？"],
			[100, f"{name}さんは処女の方が好きなんですか？"],
			[100, "処女も結構めんどくさ・・・げふんげふん"],
			[100, "処女膜破れたからって絶対に血が出るわけじゃないんですって"],
			[100, "非処女差別をやめろー！！"],
			[100, "処女なのにビッチな子もいいと思いませんか？"],
			[100, "処女まんこに精液ぶっかけまくれば処女のまま妊娠できるんですかね？"]
		]
		return get_quotes(quotes)
	...
	if "童貞" in m.content:
		quotes = [
			...
			[100, f"ま、まさか{name}さん童貞なんじゃ・・・！？"],
			[100, f"{name}さん童貞なのー？ふふん、じゃーあ・・・もらってあげてもいいですよ！・・・え？じょ、冗談なんですけど！？"],
			[100, "童貞なら旗手ができますよ！弾に当たらないんですって！"],
			[100, "やーい童貞♪童貞♪"],
			[100, "童貞なんかにこの私が負けるわけないでしょ！"],
			[100, "ど、童貞！？ﾌﾟｰｸｽｸｽ"],
			[50, "董卓♪董卓♪"],
			[100, f"{name}さんまさか童貞なの！？げ、元気だして・・・ね？"]
		]
		return get_quotes(quotes)

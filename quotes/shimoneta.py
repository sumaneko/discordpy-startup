import random
from mylib.mymodule import get_quotes
from mylib.mymodule import get_nick

#下ネタ
def get_shimoneta(m):
	t = ""
	name = get_nick(m)
	#コピペ用[100, ""],
	if "リョナラー" in m.content:
		quotes = [
			[100, f"{name}さんもリョナラーなんですか？"],
			[100, "ここにはリョナラーしかいないな！？"]
		]
		return get_quotes(quotes)

	if "グロ" in m. content:
		quotes = [
			[100, "グロいのいいですよね、内臓とかっ"],
			[100, "リョナどころかグロいやつが好きなんですか？"],
			[100, "グロにモザイク掛けなきゃいけないこの世の中を許すなーーー！！"]
		]
		return get_quotes(quotes)
	
	if "リョナ" in m.content:
		#あまりに反応するから確率を下げる
		if random.randint(0, 99) < 50:
			quotes = [
				[100, "リョナしゅき！"],
				[100, "リョナが好きだなんてすっっごい変態さんですね！"],
				[50, "ここならいいんですけど、リョナの話題はてぃーぴーおーを弁えてくださいね"],
				[100, "女の子が痛めつけられるところとか・・・フフ、興奮しちゃうんですよね"],
				[50, "版権キャラをリョナる時は色々慎重に、注意してくださいね？え、私・・・？"],
				[100, "リョナっていいのは非現実の子だけですからね？"],
				[100, "かわいすぎると虐めたくなっちゃう現象を「キュートアグレッション」って言うんですって！"],
				[100, f"{name}さんも好きですか？リョナ"]
			]
			return get_quotes(quotes)

	if "虐待" in m.content:
		quotes = [
			[100, "動物虐待ダメ絶対！！あ、美少女ならいいです"],
			[100, "リョナよりも虐待の方がなんか重い感じがしませんか？"],
			[100, "虐待ってなんかかわいそう・・・でも好き"]
		]
		return get_quotes(quotes)

	if "アヌス" in m.content:
		return "アヌスってアナルの名詞だって知ってました？でももう普通に名詞として使われてますよね"

	if "アナニー" in m.content or "アナルオナニー" in m.content:
		quotes = [
			[100, "アナニーでお尻のトレーニングをするんですか？"],
			[100, "アナニー！！"],
			[100, "男の子も女の子も出来るアナニー・・・これぞ男女平等ですね"],
			[100, "アナニーだけでイケるようになるにはかなりの熟練度が必要なんですよ"]
		]
		return get_quotes(quotes)

	if "アナル" in m.content or "肛門" in m.content or "ケツ穴" in m.content or "ケツの穴" in m.content or "けつの穴" in m.content or "お尻の穴" in m.content or ":anal:" in m.content:
		quotes = [
			[100, "アナル！"],
			[50, "アナル！！"],
			[100, f"{name}さんもアナル好きなんですか？"],
			[100, "お尻は繊細ですから、優しく使わないと切れちゃいますよ？"],
			[100, "指3本入ればおちんちんが入れられるらしいですよ！"],
			[100, "アナルに挿入してもおまんこが空いてるのがいいんですよ！"],
			[100, "ペットボトルくらいから人工肛門が必要になってくるらしいですよ"],
			[100, "アナルもののAVを撮る時、前々日とかから固形物を食べずにゼリー食で済ます女優さんがいるらしいですよ。ジャンルが変わっちゃうから！プロ根性ですね"],
			[100, "またアナルの話してる・・・"],
			[100, "いいですよね、アナル"]
		]
		return get_quotes(quotes)

	if "浣腸" in m.content:
		quotes = [
			[100, "お尻で遊ぶなら浣腸した方がいいですよ！私はうんちしないからいつでもキレイですけどねっ！"],
			[100, "牛乳浣腸って見た目はきれいなんですけど残っちゃうと腐ったりするんですって"],
			[100, "お浣腸はぬるま湯とかが無難でいいと思いますよ"]
		]
		return get_quotes(quotes)

	if "結腸" in m.content or "直腸" in m.content:
		quotes = [
			[100, "腸内に出されると後でお腹痛くなっちゃうんですよね・・・"],
			[100, "結腸超えってロマンですよね・・・"]
		]
		return get_quotes(quotes)

	if "変態" in m.content:
		quotes = [
			[100, "変態！変態！"],
			[100, "変態♪変態♪"],
			[100, "変態ですって！ウケる！"],
			[100, f"{name}さんは変態さんですか？"]
		]
		return get_quotes(quotes)

	if "セックス" in m.content or "おせっせ" in m.content or "sex" in m.content or "SEX" in m.content:
		quotes = [
			[100, "せっくす！"],
			[100, "え、セックス？・・・ドキドキ"],
			[100, "おまんこに射精すると赤ちゃんが作れるんですよ！"],
			[100, "赤ちゃん作っちゃうんですか・・・？"],
			[100, "セッ・・・や、優しくしてね？"]
		]
		return get_quotes(quotes)

	if "エッチ" in m.content or "えっち" in m.content or ":elturo:" in m.content or ":eroi:" in m.content:
		quotes = [
			[100, "今エッチな話しました？"],
			[100, "わわ・・・エッチな話してる・・・"],
			[100, "エッチしたいんですか？いや私とじゃなく・・・"],
			[100, f"もー、{name}さんエッチなんだからっ"]
		]
		return get_quotes(quotes)

	if ("すじ" in m.content and "あらすじ" not in m.content) or ":suji:" in m.content:
		quotes = [
			[100, "すじ！"],
			[100, "すじは単線ならモザイクいらないって聞きますよ"],
			[100, "つるつるきれいなすじまんこ！"],
			[100, "<:suji:864157257175531560> "],
			[100, "すじはいいですよね！"]
		]
		return get_quotes(quotes)

	if "まんこ" in m.content or "マンコ" in m.content or "まんまん" in m.content or "陰唇" in m.content or ":omanko:" in m.content:
		quotes = [
			[100, "おまんこ！"],
			[100, "やだなぁエッチなんだから・・・"],
			[50, "<:omanko:1000067595690582076>"],
			[50, "<:suji:864157257175531560>"],
			[100, "おまんこなんて言ってたら引かれちゃいますよ？"]
		]
		return get_quotes(quotes)

	if "クリトリス" in m.content or "クリちゃん" in m.content or "クリ勃起" in m.content or "クリ責め" in m.content or "陰核" in m.content:
		quotes = [
			[100, "クリトリスってあの大きさでおちんちんよりたくさんの神経が走ってるんですって！"],
			[100, "クリトリスは本当に純粋に快感を感じるためだけの器官らしいですよ？"],
			[100, "敏感だから優しく弄ってあげてくださいね！"],
			[100, "クリトリスは敏感だから、乱暴にいじっちゃいけないんですよー！"]
		]
		return get_quotes(quotes)

	if "お尻" in m.content or ("ケツ" in m.content and "バケツ" not in m.content):
		quotes = [
			[100, "お尻！"],
			[100, "お尻が大きくて安産型ならいくら孕ませても大丈夫ですね！"],
			[100, "ムチムチなお尻が好きなんですか？"],
			[100, "お尻は揉んでもいいし叩いてもいいんですよ！"]
		]
		return get_quotes(quotes)

	if "ちっぱい" in m.content or "貧乳" in m.content or "まな板" in m.content:
		quotes = [
			[100, "ちっぱい！"],
			[100, "ちっぱいちっぱい"],
			[100, "小さいおっぱいもまたかわいいですよね！"],
			[100, f"{name}さんは小さなおっぱいの方が好きなんですか？"]
		]
		return get_quotes(quotes)
	
	if "おっぱい" in m.content or "乳首" in m.content or "巨乳" in m.content or ":oppaioppai:" in m.content:
		quotes = [
			[100, "おっぱい！"],
			[100, "おっぱいおっぱい"],
			[50, "<:oppaioppai:806106303025643531> "],
			[100, "私もおっぱいにはちょっと自信があるんですよ！"],
			[100, f"{name}さんっておっぱい大好きですよね"]
		]
		return get_quotes(quotes)

	if "ふたなり" in m.content or "両性具有" in m.content:
		quotes = [
			[100, "おちんちんもおまんこも付いてるのはちょっと欲張りすぎやしませんか？"],
			[100, "タマタマが付いてるタイプはおまんこが隠れちゃうからちょっと嫌です・・・"],
			[100, "何か便利な魔法とかお薬で生えちゃうんですか？おちんちん！"]
		]
		return get_quotes(quotes)

	if "潮吹き" in m.content or "お潮" in m.content:
		quotes = [
			[100, "いっぱいお潮吹いてほしいですよね"],
			[100, "お潮吹いたからイったというわけではないんですよ"],
			[100, "潮吹きしやすくする訓練法があるんだとか"]
		]
		return get_quotes(quotes)

	if "尿道" in m.content:
		quotes = [
			[100, "尿道にはおちんちん入らないんですよ！"],
			[100, "尿道で遊ぶのは結構リスクがあるので気を付けてくださいね"],
			[100, "尿道拡張ってロマンがありますよね・・・"],
			[100, "おしっこするための穴なのに、そんなとこでも気持ちよくなれちゃうんですか？"],
			[100, "尿道突き抜けて膀胱まで犯されちゃうんだ・・・"]
		]
		return get_quotes(quotes)

	if "失禁" in m.content or "おもらし" in m.content or "お漏らし" in m.content:
		quotes = [
			[100, "いい歳しておしっこ漏らしちゃうなんて恥ずかしいですよねー！"],
			[100, "恐怖のあまりおもらししちゃうんですか？"],
			[100, "おしっこでパンツがビチョビチョになっちゃう！"]
		]
		return get_quotes(quotes)

	if "おしっこ" in m.content or "シッコ" in m.content or "小便" in m.content or "尿" in m.content:
		quotes = [
			[100, "おしっこ！"],
			[100, "飲みますか？おしっこ"],
			[100, "直飲みは無菌と言われてましたが実はそんなこと無いらしいですよ"]
		]
		return get_quotes(quotes)

	if "百合" in m.content or "レズ" in m.content or "女の子同士" in m.content:
		quotes = [
			[100, "女の子同士でエッチしちゃうんですか？"],
			[100, "女の子もかわいい女の子が好きなんですよ、かわいいから"],
			[100, "メス交尾ってやつですよね"],
			[100, "美人のお姉さんとかだったらレイプされても・・・い、いいかも・・・"],
			[100, "女の子同士なら避妊する必要がないんですよ！"],
			[100, "女の子同士でチューしちゃったりとか！？"]
		]
		return get_quotes(quotes)

	if "パチンコ" in m.content:
		quotes = [
			[100, "パチンコとチンコが別物なのくらい知ってますよ！"],
			[100, "パチンコにチンコがついてるからって喜んだりしませんよ！"],
			[100, "またパチンコで負けちゃったのー？？ﾌﾟｰｸｽｸｽ"]
		]
		return get_quotes(quotes)

	if "ちんこ" in m.content or "チンコ" in m.content or "チンチン" in m.content or "ちんちん" in m.content or "チンポ" in m.content or "ちんぽ" in m.content or ":chinpo:" in m.content:
		quotes = [
			[100, "え、おちんちん・・・？"],
			[100, "ちーんちん"],
			[100, "おちんちんの話してます？ウケるー！"],
			[100, "おちんちんの長さは定規を恥骨に押し付けて測るんですよ、知ってました？"],
			[50, "<:standup:805846231851073647>"],
			[100, "ち・・・チンポ"]
		]
		return get_quotes(quotes)

	if "ペニス" in m.content:
		quotes = [
			[100, "つまりおちんちんのことですよね？"],
			[100, "おペニス・・・・・・語呂が悪いな"],
			[100, "おちんちんの長さは定規を恥骨に押し付けて測るんですよ、知ってました？"]
		]
		return get_quotes(quotes)

	if "キンタマ" in m.content or "金玉" in m.content or "タマタマ" in m.content:
		quotes = [
			[100, "本当に金で出来てるのか切開して確かめてもいいですか？"],
			[100, "タマタマを舐めて欲しいんですか？"],
			[100, "金玉って球体じゃなくてラグビーボールみたいな楕円形なんですって！"],
			[100, "金玉って50kg程度で潰れちゃうんですって！大切にしてあげてくださいね！"],
			[100, "急所なのにタマタマが外に出てるのは、精子が34度付近が一番生産しやすく活動的になるからなんですよ"],
			[100, "金玉って蹴られるとめっちゃ痛いんでしょー？どんな感じなんですか！"],
			[100, "左右で玉の大きさが違うから位置が逆にならないらしいですよ。でもソースは忘れちゃったので違っても知りません"]
		]
		return get_quotes(quotes)

	if ("ロリ" in m.content and "カロリー" not in m.content and "ブロリー" not in m.content and "ポロリ" not in m.content) or "幼女" in m.content:
		quotes = [
			[100, "いいですよねロリ"],
			[100, "幼女！幼女！"],
			[100, "ロリ好き！"],
			[100, "このロリコン！"]
		]
		return get_quotes(quotes)

	if "肉便器" in m.content or "便女" in m.content:
		quotes = [
			[100, "肉便器とか公衆便女とかひどいことしますよね全くぅ・・・"],
			[100, "誰のかも分からない赤ちゃんができちゃう！"],
			[100, "公衆便所に放置しておしっことか掛けてあげると更に屈辱的でいいと思います"]
		]
		return get_quotes(quotes)

	if "ビッチ" in m.content or "淫乱" in m.content or "淫売" in m.content:
		quotes = [
			[100, "ビッチな子が好きなんですか？"],
			[100, "人間の三大欲求に性欲がある以上　エッチなことが好きなのはしょうがないですよね・・・"],
			[100, "私は淫乱じゃないですよ！"]
		]
		return get_quotes(quotes)

	if "中出し" in m.content:
		quotes = [
			[100, "中出しし続けると1年で85%の確率で妊娠するらしいですよ"],
			[100, "あ、赤ちゃんできちゃうぅぅーー！！ってやつですよね！"],
			[100, f"{name}さんも中出し派ですか？"]
		]
		return get_quotes(quotes)

	if "外出し" in m.content:
		quotes = [
			[100, "だ、出すなら外に出して！！ってやつですよね！"],
			[100, "外出ししても妊娠すると言いますが、確実に行えれば妊娠率が4%まで下がるんですって"],
			[100, "外出ししてぶっかけちゃうんですか？"]
		]
		return get_quotes(quotes)


	if "触手貫通" in m.content:
		quotes = [
			[100, "触手でお尻だけじゃなく内臓とお口まで犯されちゃうなんて・・・欲張り！"],
			[100, "口から突っ込んでお尻から出てくる派ですか？それともお尻から挿入れちゃいますか？"],
			[100, "丁寧に進ませないと内臓が破けちゃいますよ！"],
			[100, "つ【https://ux.getuploader.com/znzn_loader/download/4 】（ZNZNさん製「触手貫通イライラ棒」FLASH版はもうできません）"],
			[100, "触手が貫通しても死なないなんてすごいですよね！"]
		]
		return get_quotes(quotes)

	if "触手" in m.content:
		quotes = [
			[100, "触手はいいですよね・・・色んなことができて・・・"],
			[100, "余ってる触手をお尻に突っ込めーーー！ア●●●●を●●なーーーー！！"],
			[100, "触手ってすごいですよね・・・おちんちんじゃ届かないところまでいけちゃうんですよ・・・？"]
		]
		return get_quotes(quotes)

	if "輪姦" in m.content:
		quotes = [
			[100, "たくさんの男の人にもみくちゃにされて・・・体中弄ばれちゃうんだ・・・"],
			[100, "おちんちんが余ってるならお尻も使ってくださいね"],
			[100, "竿が多すぎるとちょっと大変ですよね色んな意味で"]
		]
		return get_quotes(quotes)

	if "レイプ" in m.content or "強姦" in m.content:
		quotes = [
			[100, "無理矢理女の子を犯しちゃいけないんだぞー"],
			[100, "レイプされても心までは折れないんだからっ！"],
			[100, "汚いおじさんにレイプされるのは嫌だけど、かっこいい人とか女の子だったらまあ・・・いいかも・・・"]
		]
		return get_quotes(quotes)

	if "拡張" in m.content:
		quotes = [
			[100, "おまんこもアナルも尿道まで拡張出来るらしいですよ、人体ってすごい！"],
			[100, "拡張って言葉、ロマンがありますよね・・・"],
			[100, "拡張されて大きいおちんちんを挿入されちゃうんだ・・・"]
		]
		return get_quotes(quotes)

	if "ボコォ" in m.content:
		quotes = [
			[100, "大きすぎるおちんちんで外から見ても分かるくらいお腹が拡がっちゃう！"],
			[100, "モンスターとかお馬さんのおちんちんでボコォってなっちゃうの好き"],
			[100, "お腹が変形するほど大きいおちんちん・・・すごそう"]
		]
		return get_quotes(quotes)

	if "オナホ妖精" in m.content:
		quotes = [
			[100, "妖精さんに人間のおちんちんが入るわけないでしょ！・・・え？入らなくても挿入れる？"],
			[100, "妖精さんは体が柔らかいのでヒトサイズのおちんちんも入るらしいですよ"],
			[100, "妖精さんってかわいいですよね・・・え？オナホ？"]
		]
		return get_quotes(quotes)

	if "オナホ" in m.content:
		quotes = [
			[100, "オナホオナホって、オナホにされる女の子の気持ち考えたことあるんですか！ちょっと気持ちよさそうですけど！"],
			[100, f"ﾌﾟﾌﾟｰ、{name}さんオナホなんかで満足してるんですか？\n・・・あっ、冗談ですよ？わわっ！ちょ、押し倒さないでっ！"],
			[100, "軽い子は簡単に持ち上がるからすぐオナホにされちゃう"]
		]
		return get_quotes(quotes)

	if "オナ" in m.content:
		quotes = [
			[100, "オナニー！"],
			[100, "お尻も一緒にいじった方が気持ちいいですよ！"],
			[100, "え、ここでオナニーしろ、ですって・・・？"]
		]
		return get_quotes(quotes)

	if "シコ" in m.content or ":sikosiko:" in m.content:
		quotes = [
			[100, "シコシコしてほしいんですか？"],
			[100, "シッコーーいッ！"],
			[100, "しごいてあげましょうか？"]
		]
		return get_quotes(quotes)

	if "パンツ" in m.content:
		quotes = [
			[100, f"{name}さんは何色のおパンツが好きですか？"],
			[100, "冷静に考えると、たった1枚の布でお股を隠してるのってエッチですよね・・・？"],
			[100, "ノーパン健康法ってあながち嘘じゃないらしいですよ"],
			[100, "尻尾がある女の子のパンツって、それぞれ工夫がしてあってかわいいんですよ！"]
		]
		return get_quotes(quotes)

	if "ドM" in m.content or ("マゾ" in m.content and "アマゾン" not in m.content):
		quotes = [
			[100, f"{name}さんはドMなんですか？"],
			[100, "リョナラーとしては虐めてるのに悦んでもらっちゃぁ困りますよね・・・"],
			[100, "創作活動はマゾいつまり製作者はみんなマゾ！"]
		]
		return get_quotes(quotes)

	if "ドS" in m.content or "サド" in m.content:
		quotes = [
			[100, "リョナラーたるものみんなきっとサドなんですよ！"],
			[100, "や、優しくしてね・・・？"],
			[100, "SMってSの方が気を遣うんですって！"]
		]
		return get_quotes(quotes)

	if "クンニ" in m.content:
		quotes = [
			[100, "まんまんぺろぺろ"],
			[100, "クンニ！"],
			[100, "ぺろぺろしたいんですか？"]
		]
		return get_quotes(quotes)

	if "イラマ" in m.content:
		quotes = [
			[100, "おちんちんを喉奥に突っ込まれると本当に苦しいんですからねっ"],
			[100, "頭をしっかり掴まれておちんちんを打ち付けられちゃったら、それはもう人間オナホですよね"],
			[100, "無理やりおちんちん突っ込まれてオエェってなっちゃうんだ・・・"]
		]
		return get_quotes(quotes)

	if "フェラ" in m.content:
		quotes = [
			[100, "おちんちん・・・舐めさせられるの苦手なんですよね・・・"],
			[100, "ぺろぺろされるの好きなんですか？"],
			[100, "どうせ頭を掴んでイラマする気なんでしょ！"]
		]
		return get_quotes(quotes)

	if "異種姦" in m.content or "モンスター姦" in m.content:
		quotes = [
			[100, "ひ、ヒトじゃない化け物に犯されちゃうの・・・！？"],
			[100, "異種族なだけあっておちんちんも人間サイズじゃないんだ！きっと！"],
			[100, "異種姦にハマってラブラブになると異種和姦になっちゃうんですって"]
		]
		return get_quotes(quotes)

	if "獣姦" in m.content:
		quotes = [
			[100, "わんことかお馬さんのおちんちんで犯されちゃうんだ・・・"],
			[100, "人間なのに動物の交尾をしちゃうんですか？"],
			[100, "人間では味わえないおちんちんを持ってるんですかね・・・？"]
		]
		return get_quotes(quotes)

	if "犬姦" in m.content:
		quotes = [
			[100, "おちんちんの根本、亀頭球が膨らんで精液が出れなくなっちゃうんですよね"],
			[100, "交尾結合！わんこに犯されるなら交尾結合まで描写してください！"],
			[100, "おまんこにバターを塗れば舐めちゃうってほんとー？"]
		]
		return get_quotes(quotes)

	if "馬姦" in m.content:
		quotes = [
			[100, "お馬さんのおちんちんなんて入るわけないでしょ！"],
			[100, "おちんちんだけで私の腕より大きいんじゃないですか？"],
			[100, "お馬さんだけあって精液もきっとたくさん出るんだ・・・"]
		]
		return get_quotes(quotes)

	if "豚姦" in m.content:
		quotes = [
			[100, "豚さんは亀頭部分が細いドリルみたいになってるんですって！"],
			[100, "子宮口をこじ開けられて直接射精された後、入り口を固められてフタされちゃうんだ・・・"],
			[100, "豚さんってタマタマもすごく大きいし射精量もいっぱい出るらしいですよ"]
		]
		return get_quotes(quotes)

	if "機械姦" in m.content:
		quotes = [
			[100, "何の感情も無い機械に犯されて作業的に絶頂させられちゃうんだ・・・"],
			[100, "何回イってもエネルギーが切れるまでやめてくれずに無限に犯されちゃうんですか？"],
			[100, "機械は触手と違って無機物で非生物なのがポイントですかね"]
		]
		return get_quotes(quotes)

	if "蟲姦" in m.content or "虫姦" in m.content:
		quotes = [
			[100, "虫さんに種付けされちゃうんですか？"],
			[100, "虫さんの場合精子じゃなくて卵とか産み付けてくるんですよね"],
			[100, "虫チンポで機械的に托卵されちゃうんだ・・・"]
		]
		return get_quotes(quotes)

	if "子宮" in m.content:
		quotes = [
			[100, "子宮口って普通拡がらないから本当はおちんちん入らないんですよ！"],
			[100, "子宮が疼くー、なんてことほんとにあるんですかー？"],
			[100, "おちんちんが大きすぎて子宮まで犯しちゃうんですか？"],
			[100, "子宮口の向きって膣道に対して直角についてるんですよ、知ってました？"],
			[100, "まさに女の子の象徴だと思います、子宮"]
		]
		return get_quotes(quotes)

	if "腋" in m.content and "腋毛" not in m.content:
		quotes = [
			[100, f"{name}さんって腋フェチなんですか？"],
			[100, "腋部分が空いてる衣装ってエッチですよね・・・"],
			[100, "腋をペロペロしたいんですか？"],
			[100, "腋もエッチなので性器みたいなものなんですよ！"]
		]
		return get_quotes(quotes)

	if "貝合せ" in m.content:
		quotes = [
			[100, "おまんこ同士を擦り合わせるやつですか？"],
			[100, "女の子同士のロマン体位ですよねー"],
			[100, "貝合せもけっこう気持ちよくするのが難しいらしいですよ"]
		]
		return get_quotes(quotes)

	if "処女" in m.content:
		quotes = [
			[100, "処女膜って実は色んな形があるんですよ"],
			[100, "処女のままアナルを開発してあげたーい！"],
			[100, "私も処女ですよ！！"],
			[100, "せっかく処女を守ってきたのに、レイプとかで無理矢理散らされちゃうのいいですよね！"],
			[100, "ほそーい触手を使えば処女のまま開発できるのかなぁ？"],
			[100, f"{name}さんは処女の方が好きなんですか？"],
			[100, "処女も結構めんどくさ・・・げふんげふん"],
			[100, "処女膜破れたからって絶対に血が出るわけじゃないんですって"],
			[100, "非処女差別をやめろー！！"],
			[100, "処女なのにビッチな子もいいと思いませんか？"],
			[100, "処女まんこに精液ぶっかけまくれば処女のまま妊娠できるんですかね？"]
		]
		return get_quotes(quotes)

	if "童貞" in m.content:
		quotes = [
			[100, "童貞が許されるのはー小学生までだよねー！"],
			[100, "英語だと童貞もヴァージンって言うんですよ"],
			[100, f"ま、まさか{name}さん童貞なんじゃ・・・！？"],
			[100, "童貞ですって！ウケるー！"],
			[100, f"{name}さん童貞なのー？ふふん、じゃーあ・・・もらってあげてもいいですよ！・・・え？じょ、冗談なんですけど！？"],
			[100, "童貞なら旗手ができますよ！弾に当たらないんですって！"],
			[100, "やーい童貞♪童貞♪"],
			[100, "童貞なんかにこの私が負けるわけないでしょ！"],
			[100, "ど、童貞！？ﾌﾟｰｸｽｸｽ"],
			[50, "董卓♪董卓♪"],
			[100, f"{name}さんまさか童貞なの！？げ、元気だして・・・ね？"]
		]
		return get_quotes(quotes)

	if "精子" in m.content or "精液" in m.content or "ザーメン" in m.content or "ザー汁" in m.content:
		quotes = [
			[100, "精子一匹の全長って0.06mmで髪の毛の直径くらいなんですって！頭の部分は0.005mmらしいですよ"],
			[100, "卵子の大きさは精子の頭の部分の長さの30倍以上とかなんですよ、受精描写する時の参考にしてくださいね"],
			[100, "現実のザーメンって一回5mlとかしか出ないんですって、つまんなーい"],
			[100, "射精で出る液体の中に精子は1%とかしか入ってなくて、後は精漿っていうやつなんですよ"],
			[100, "精液を100ml飲むとタンパク質が5g摂れるんですよ！亜鉛も豊富なんですって"],
			[100, "飲めと言われても飲むのはちょっと苦手ですね・・・"],
			[100, "エロ漫画とかエロゲーみたいにリットル単位で出して欲しいー"],
			[100, "な、中はだめーッ！出すなら外に出して！！・・・なんちゃって♪"],
			[100, "精液は栄養あるけど量が足りないーって言うなら、人間の数を増やせばいいじゃない！"],
			[100, "すーぐ精子飲ませたがるけど喉に絡んでかなりキツいんですからね"]
		]
		return get_quotes(quotes)

	if "愛液" in m.content or "マン汁" in m.content or "まん汁" in m.content:
		quotes = [
			[100, "精液も愛液も人によって、あと体調によってかなり味が違うらしいですよ！"],
			[100, "人によって塗れやすい子も濡れにくい子もいるんですよ"],
			[100, "レイプされて感じてるから濡れるんじゃなくて、反射反応で濡れちゃうだけなんですからね！"],
			[100, "中でお汁がかき混ぜられてクチュクチュ言っちゃうんだ・・・"],
			[100, "愛液が太ももを伝ってテラテラ光ってるの・・・エッチでいいと思いませんか？"],
			[100, "愛の液、なんだか幸せになる響きですよね"],
			[100, "お腹殴られたりすると濡れちゃうのも、反射反応なだけで私がマゾなわけじゃ・・・"]
		]
		return get_quotes(quotes)

	if "嫌パン" in m.content or "嫌な顔しながらおパンツ見せてもらいたい" in m.content:
		quotes = [
			[100, "パンツが見たいんですか？"],
			[100, "私に嫌パンしてもらいたいんですか？"],
			[100, "本当に嫌だからおパンツなんて見せてあげませんよーだ！！"]
		]
		return get_quotes(quotes)
	
	if "催眠" in m.content:
		quotes = [
			[100, "催眠アプリなんて引っかかるわけ無いですよねーあんなの！"],
			[100, "エッチになる催眠なんて本当にあるんですかー？うっそだー"],
			[100, "催眠術なんてきっとやらせでしょ～？ウケるー！"]
		]
		return get_quotes(quotes)
	
	return t
"""
if "" in m.content:
		quotes = [
			[100, ""],
			[100, ""],
			[100, ""]
		]
		return get_quotes(quotes)
"""